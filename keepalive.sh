#!/bin/bash
# ÁGUIA Keepalive v2 — Hardened
# Runs every 5 min via cron. Ensures the agent is alive, responsive, and patched.
...
if tmux has-session -t aguia 2>/dev/null; then
    ...
    if [ "$HOUR" -ge 6 ] && [ "$HOUR" -le 22 ]; then
        if [ -n "$CLAUDE_PID" ]; then
            # Check CPU usage -- 0% for extended time means stuck
            CPU=$(ps -p "$CLAUDE_PID" -o %cpu= 2>/dev/null | tr -d ' ' | cut -d. -f1)
            if [ "${CPU:-0}" -eq 0 ]; then
                # Check if it's been idle for a while by looking at process state
...
# Ensure critical users are always on the allowlist (prevents lockouts on restart)
ACCESS_FILE="/Users/bruno.pessoa/.claude/channels/telegram/access.json"
if [ -f "" ]; then
    python3 -c "
import json
with open('') as f:
    cfg = json.load(f)
changed = False
for uid in ['5138814159', '8793688693']:  # Bruno, Angelika
...
# ── 8. WHATSAPP WEBHOOK HANDLER HEALTH ────────────────────────────────────────
# Ensure WA webhook handler is running on port 8086 (check port, not process name)
if ! lsof -i :8086 -sTCP:LISTEN > /dev/null 2>&1; then
    source "$HOME/aguia/.wa-webhook.env" 2>/dev/null
    export WASENDER_SESSION_KEY WASENDER_WEBHOOK_SECRET
    cd "$HOME/aguia" && \
    nohup python3 scripts/wa-webhook-handler.py \
        >> "$HOME/aguia/shared/logs/wa-webhook.log" 2>&1 &
    echo "$(date): Restarted WA webhook handler" >> "$LOG"
fi
